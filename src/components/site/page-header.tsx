  ...
  return (
    <div className={cn("relative border-b border-[var(--border)]", className)}>
      ...
      <div className="ui-site-container relative pb-12 pt-16 text-center sm:pb-16 sm:pt-20 sm:text-left">
        {kicker && (
          <FadeIn duration={0.6}>
            <p className="mb-6 font-mono text-[0.6875rem] font-medium uppercase tracking-[0.18em] text-[var(--muted-foreground)]">
              {kicker}
            </p>
          </FadeIn>
        )}
        <FadeIn duration={0.7} delay={kicker ? 0.1 : 0}>
          <h1 className="font-display text-[clamp(2.5rem,5.5vw,5rem)] font-semibold leading-[1.05] tracking-[-0.03em] text-balance text-[var(--foreground)]">{title}</h1>
        ...
        {(subtitle || children) && (
          <FadeIn delay={0.25} direction="up" distance={20}>
            <div className="mt-6">
              {subtitle && (
                <p className="mx-auto max-w-2xl text-base leading-relaxed text-[var(--muted-foreground)] sm:mx-0 sm:text-[1.05rem]">
                  {subtitle}
                </p>
              )}
              {children}
