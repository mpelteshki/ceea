  ...
  return (
    <div className="relative z-50" ref={containerRef}>
      ...
      {isOpen ? (
        <div
          id={menuId}
          className="absolute right-0 top-full mt-2 w-40 overflow-hidden rounded-lg border border-[var(--accents-2)] bg-[var(--background)] shadow-lg animate-in fade-in slide-in-from-top-1 duration-150"
        >
          <div className="p-1" role="listbox" aria-label="Available languages">
            {locales.map((l) => (
              <button
                ...
                className={cn(
                  "relative flex w-full items-center justify-between rounded-md px-3 py-2 text-sm text-[var(--foreground)] transition-[background-color,transform] hover:bg-[var(--accents-1)] hover:translate-x-0.5",
                  locale === l.code && "bg-[var(--accents-1)] font-medium",
                )}
              >
                {l.label}
                {locale === l.code ? <Check className="h-3 w-3" /> : null}
              </button>
            ))}
          </div>
        </div>
